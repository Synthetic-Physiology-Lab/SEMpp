...
# flags = more flags. (first, last, step)
proc ::TopoTools::readvarxyz {filename {flags {}}} {
    ...
    set nframes    0 ; # total number of frames that were kept
    set nframesF   0 ; # total number of frames that were in file
    ...
    vmdcon -info "readvarxyz: will keep $first : $step : $last frames"
    ...
    while {[gets $fp line] >= 0} {
        ...
        # skip frame?
        if {$nframesF >= $first && $nframesF <= $last && ($nframesF - $first) % $step == 0} {
        	vmdcon -info "readvarxyz: keeping frame $nframesF"
